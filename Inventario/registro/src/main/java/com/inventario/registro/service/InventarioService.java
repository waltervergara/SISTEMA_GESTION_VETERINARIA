...
public class InventarioService {
    ...
    public Inventario actualizarInventario(String nombre , Inventario inventarioExistente){
        ...
        if (inventarioExistente.getNombre() == null || inventarioExistente.getNombre().trim().isEmpty()) {
                throw new IllegalArgumentException("El nombre es obligatorio.");
        }

        if (inventarioExistente.getFecha_elaboracion() == null ) {
                throw new IllegalArgumentException("la fecha es obligatorio.");
        }

        if (inventarioExistente.getStock() == null ) {
                throw new IllegalArgumentException("El Stock es obligatorio.");
        }

        if (inventarioExistente.getVencimiento() == null || inventarioExistente.getVencimiento().trim().isEmpty()) {
                throw new IllegalArgumentException("la fecha de vencimiento es obligatorio.");
        }

        if (inventarioExistente.getDescripcion() == null || inventarioExistente.getDescripcion().trim().isEmpty()) {
                throw new IllegalArgumentException("la descripcion es obligatorio.");
        }

        if (inventarioExistente.getPrecio() == null) {
